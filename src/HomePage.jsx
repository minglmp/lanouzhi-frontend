        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredModels.map((model) => (
              <div 
                key={model.id} 
                onClick={() => navigate(`/model/${model.id}`)} // 👇 เพิ่มคำสั่งนำทาง
                className="group bg-gray-900 rounded-3xl overflow-hidden border border-gray-800 shadow-md hover:shadow-2xl transition-all duration-300 flex flex-col relative cursor-pointer" // 👇 เพิ่ม cursor-pointer
              >
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto">
                    <button onClick={(e) =>{ e.stopPropagation(); handleOrder(model.title)}} className="w-full bg-[#FF7518] hover:bg-orange-600 text-white font-bold py-2.5 rounded-xl transition-colors shadow-sm hover:shadow-md flex justify-center items-center gap-2">
